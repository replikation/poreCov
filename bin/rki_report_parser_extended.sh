...
while IFS=$'\t' read -r -a Array; do
    SENDING_LAB=$(grep "${Array[1]}" ${EXTENDED_TABLE} | cut -f 2 -d ",")
    DATE_DRAW=$(grep "${Array[1]}" ${EXTENDED_TABLE} | cut -f 3 -d ",")
    SEQU_TECH=$(echo "OXFORD_NANOPORE")
    SEQU_REASON=$(grep "${Array[1]}" ${EXTENDED_TABLE} | cut -f 4 -d ",")
    SAMPLE_TYPE=$(grep "${Array[1]}" ${EXTENDED_TABLE} | cut -f 5 -d ",")
